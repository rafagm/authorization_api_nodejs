...
router.post('/register', async (req, res) => {
    ...
    if (userWithExistingEmail) return res.status(400).send("The email already exists");
...
router.post('/login', async (req, res) => {
    ...
    if (!user) return res.status(400).send(vagueResponseMessage);

    const passwordsMatch = await bcrypt.compare(req.body.password, user.password);
    if (!passwordsMatch) return res.status(400).send(vagueResponseMessage);
